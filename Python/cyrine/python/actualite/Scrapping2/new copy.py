...
def actualites_bourse_tunis():
    ...
    return render_template('actualites_bourse_tunis.html', articles=articles,  mimetype='text/html')
...
def scrape_actualites(url, output_file="templates/actualites_content.html"):
    response = requests.get(url)
    if response.status_code == 200:
        content = response.text
        write_to_file(content, output_file)
...
def actualites():
    ...
    if actualites_content:
        return render_template('actualites_content.html', actualites_content=actualites_content)
    else:
